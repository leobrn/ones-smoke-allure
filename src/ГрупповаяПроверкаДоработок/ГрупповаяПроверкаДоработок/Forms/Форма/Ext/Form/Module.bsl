...
Процедура ОбъектыРасширенний(Команда)
	...
	Печать_ПроставитьЗначение(Истина);
...
&НаКлиенте
Процедура Печать_ПроставитьЗначение(ТолькоОбъектыРасширений = Ложь)
	...
	Для Каждого СтрокаКорня Из Печать_ДеревоОбъектов.ПолучитьЭлементы() Цикл
		...
		Для Каждого СтрокаМетаданного Из СтрокаКорня.ПолучитьЭлементы() Цикл
			...
			Для Каждого СтрокаПечатнойФормы Из СтрокаМетаданного.ПолучитьЭлементы() Цикл
				...
				Тестировать = ?(ЗначениеЗаполнено(СтрокаПечатнойФормы.ОбъектПечати), 1, 0);
				УстановитьПризнакТестирования(СтрокаПечатнойФормы, Тестировать, ТолькоОбъектыРасширений);
