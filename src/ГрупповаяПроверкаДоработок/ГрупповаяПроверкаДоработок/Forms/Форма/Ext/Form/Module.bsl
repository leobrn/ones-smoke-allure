...
Процедура ЗаполнитьДеревоИнформации()
	...
	Дерево = ТаблицаЗначенийВДеревоЗначений(_Информация.Выгрузить(), "ТипОшибки");
...
&НаКлиентеНаСервереБезКонтекста
Функция РазницаВМилисекундахМеждуЮниксИНачалЭпохи()
	Возврат 62135596800000;
КонецФункции

&НаСервере
Функция ТаблицаЗначенийВДеревоЗначений(ТаблицаЗначений, ГруппировкаКолонка) Экспорт 
   ...
   Построитель.ИсточникДанных.Колонки[ГруппировкаКолонка].Измерение = Истина;
...
Процедура СоздатьОтчетАллюр()
	...
	Отчет = СоздатьОтчетАллюрНаСервере();
...
Функция СоздатьОписаниеШага(Знач РезультатШага)
	...
	РазницаВМилисекундах = РазницаВМилисекундахМеждуЮниксИНачалЭпохи();
	
	ОписаниеШага.Вставить("name"				, РезультатШага.Представление);
	ОписаниеШага.Вставить("description"			, РезультатШага.ИмяМетода);

	ОписаниеШага.Вставить("start"				, РезультатШага.ВремяНачала - РазницаВМилисекундах);
	ОписаниеШага.Вставить("stop"				, РезультатШага.ВремяОкончания - РазницаВМилисекундах);

	ВставитьОписаниеСтатусаАллюр(ОписаниеШага	, РезультатШага.Сообщение);
	...
	ОписаниеШага.historyId = СтрШаблон("%1.%2.%3", ИмяФичи, ИмяСтори, РезультатШага.Представление);
