...
Процедура ОбъектыИзмененные(Команда)
	...
	Формы_ПроставитьЗначенияПодчиненных(Формы_ДеревоОбъектов, Истина, Ложь, Истина);
	Отчеты_ПроставитьЗначениеВоВсеСтроки(Отчеты_ДеревоОбъектов, Истина, Ложь, Истина);
	Печать_ПроставитьЗначение(Истина, Ложь, Истина);
	Документы_ПроставитьЗначенияПодчиненных(Документы_ДеревоОбъектов, Истина, Ложь, Истина);
	Справочники_ПроставитьЗначенияПодчиненных(Справочники_ДеревоОбъектов, Истина, Ложь, Истина);
...
&НаКлиентеНаСервереБезКонтекста
Процедура Формы_ПроставитьЗначенияПодчиненных(Строка, Значение, ТолькоОбъектыРасширений = Ложь, ТолькоОбъектыИзмененные = Ложь)
	
	Для Каждого ПодСтрока Из Строка.ПолучитьЭлементы() Цикл
		
		УстановитьПризнакТестирования(ПодСтрока, Значение, ТолькоОбъектыРасширений, ТолькоОбъектыИзмененные);  
		
		Если ПодСтрока.ПолучитьЭлементы().Количество() <> 0 Тогда
			Формы_ПроставитьЗначенияПодчиненных(ПодСтрока, Значение, ТолькоОбъектыРасширений, ТолькоОбъектыИзмененные);
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура Документы_ПроставитьЗначенияПодчиненных(Строка, Значение, ТолькоОбъектыРасширений = Ложь, ТолькоОбъектыИзмененные = Ложь)
	...
	Для Каждого ПодСтрока Из Строка.ПолучитьЭлементы() Цикл
		...
		Если ПодСтрока.ПолучитьЭлементы().Количество() <> 0 Тогда
			Документы_ПроставитьЗначенияПодчиненных(ПодСтрока, Значение, ТолькоОбъектыРасширений, ТолькоОбъектыИзмененные);
		КонецЕсли;
...
&НаКлиенте
Процедура Печать_ПроставитьЗначение(Значение, ТолькоОбъектыРасширений = Ложь, ТолькоОбъектыИзмененные = Ложь)
	...
	Для Каждого СтрокаКорня Из Печать_ДеревоОбъектов.ПолучитьЭлементы() Цикл
		...
		Для Каждого СтрокаМетаданного Из СтрокаКорня.ПолучитьЭлементы() Цикл
			...
			Для Каждого СтрокаПечатнойФормы Из СтрокаМетаданного.ПолучитьЭлементы() Цикл
				...
				УстановитьПризнакТестирования(СтрокаПечатнойФормы, Значение, ТолькоОбъектыРасширений, ТолькоОбъектыИзмененные);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьПризнакТестирования(СтрокаОбъекта, Значение, ТолькоОбъектыРасширений = Ложь, ТолькоОбъектыИзмененные = Ложь)
	
	Если ТолькоОбъектыРасширений Тогда
		СтрокаОбъекта.Тестировать = ?(СтрокаОбъекта.ЕстьИзмененияРасширениями, Значение, НЕ Значение);
	ИначеЕсли ТолькоОбъектыИзмененные Тогда
		СтрокаОбъекта.Тестировать = ?(СтрокаОбъекта.ЕстьИзмененияПоддержки, Значение, НЕ Значение);
	Иначе
...
&НаКлиентеНаСервереБезКонтекста
Процедура Отчеты_ПроставитьЗначениеВоВсеСтроки(Узел, Значение, ТолькоОбъектыРасширений = Ложь, ТолькоОбъектыИзмененные = Ложь)
	
	Для Каждого Строка Из Узел.ПолучитьЭлементы() Цикл
		
		УстановитьПризнакТестирования(Строка, Значение, ТолькоОбъектыРасширений, ТолькоОбъектыИзмененные);
		Отчеты_ПроставитьЗначениеВоВсеСтроки(Строка, Значение, ТолькоОбъектыРасширений, ТолькоОбъектыИзмененные);
...
&НаКлиентеНаСервереБезКонтекста
Процедура Справочники_ПроставитьЗначенияПодчиненных(Строка, Значение, ТолькоОбъектыРасширений = Ложь, ТолькоОбъектыИзмененные = Ложь)
	...
	Для Каждого ПодСтрока Из Строка.ПолучитьЭлементы() Цикл
		...
		Если ПодСтрока.ПолучитьЭлементы().Количество() <> 0 Тогда
			Справочники_ПроставитьЗначенияПодчиненных(ПодСтрока, Значение, ТолькоОбъектыРасширений, ТолькоОбъектыИзмененные);
		КонецЕсли;
