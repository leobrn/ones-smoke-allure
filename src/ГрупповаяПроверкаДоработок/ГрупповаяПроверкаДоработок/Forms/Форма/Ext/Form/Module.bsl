...
Процедура ЗаполнитьДеревоИнформации()
	...
	_Информация.Сортировать("ТипОшибки");
	
	ТекущийТипОшибки			= Неопределено;
	ЭлементыКорневогоУзла		= Неопределено;
	
	Для Каждого СтрокаОшибки Из _Информация Цикл
		
		Если ТекущийТипОшибки <> СтрокаОшибки.ТипОшибки Тогда
			
			ТекущийТипОшибки	= СтрокаОшибки.ТипОшибки;
			
			СтрокаГруппы				= КорневыеУзлы.Добавить();
			СтрокаГруппы.ТипОшибки		= СтрокаОшибки.ТипОшибки;
		...
		Если ТекФорма.МассивСсылок.Количество() <> 0 Тогда
			...
			Для каждого Ключ Из ТекФорма.МассивСсылок Цикл
				...
				Исключение
					...
					НоваяСтрока_Информация.ПроверяемыйОбъект = ТекФорма.ИмяФормы;
					НоваяСтрока_Информация.Данные = Ключ;
					НоваяСтрока_Информация.Описание = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					НоваяСтрока_Информация.Картинка = БиблиотекаКартинок.Форма;
				КонецПопытки;
...
Процедура Отчеты_СформироватьФайлОтчета(Параметры)
	...
	Исключение
		...
		НоваяСтрока_Информация.ПроверяемыйОбъект = "Отчет." + Параметры.ИмяОтчета;
		...
		НоваяСтрока_Информация.Картинка = БиблиотекаКартинок.Отчеты;
...
Процедура Печать_СформироватьФайлПечатнойФормы(Параметры, ПрефиксФайла = "")
	...
	Исключение
		...
		НоваяСтрока_Информация.ПроверяемыйОбъект = ИмяИсточника;
		НоваяСтрока_Информация.Данные = ОбъектПечати;
		НоваяСтрока_Информация.Описание = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		НоваяСтрока_Информация.Картинка = БиблиотекаКартинок.Печать;
...
Процедура Документы_Проверка()
	...
	Исключение
		...
		НоваяСтрока_Информация.ПроверяемыйОбъект = "Документы";
		НоваяСтрока_Информация.Данные = Неопределено;
		НоваяСтрока_Информация.Описание = "Ошибка при выполнении тестирования"
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		НоваяСтрока_Информация.Картинка = БиблиотекаКартинок.Документ;	
...
Процедура Документы_ПротестироватьОбъектПоСсылкеНаКлиенте(ИмяОбъектаМетаданных, Ссылка)
	...
	Исключение
		...
		НоваяСтрока_Информация = _Информация.Добавить();
		НоваяСтрока_Информация.ТипОшибки = "Документы";
		НоваяСтрока_Информация.ПроверяемыйОбъект = ИмяФормыТестирования;
	...
	Для Каждого ТекДокумент Из МассивРегистраторов Цикл
		...
		Если ЕстьОшибкиЗаполнения Тогда
			...
			Для Каждого СтрокаСообщения Из СообщенияПользователю Цикл
				...
				пОписание = "Ошибка проверки заполнения " + ИмяМетаданного + ПолучитьНавигационнуюСсылку(Ключ) 
					+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				Если _Информация.НайтиСтроки(Новый Структура("Данные, Описание", Ключ, пОписание)).Количество() = 0 Тогда
					НоваяСтрока_Информация = _Информация.Добавить();
					НоваяСтрока_Информация.ТипОшибки = "Документы";
					НоваяСтрока_Информация.ПроверяемыйОбъект = ИмяМетаданного;
					НоваяСтрока_Информация.Данные = Ключ;
...
Процедура СправочникиИПВХ_Проверка()
	...
	Исключение
		...
		НоваяСтрока_Информация.ПроверяемыйОбъект = "Справочники";
		НоваяСтрока_Информация.Данные = Неопределено;
		НоваяСтрока_Информация.Описание = "При тестировании объектов не должно возникать ошибок обработки " 
			+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		НоваяСтрока_Информация.Картинка = БиблиотекаКартинок.Документ;	
...
Функция Справочники_ПротестироватьОбъектПоСсылкеНаКлиенте(ИмяОбъектаМетаданных, Ссылка)
	...
	Исключение
		...
		НоваяСтрока_Информация = _Информация.Добавить();
		НоваяСтрока_Информация.ТипОшибки = "Справочники";
		НоваяСтрока_Информация.ПроверяемыйОбъект = Справочники_ПолучитьИмяОсновнойФормыОбъектаОбъектаМетаданных(ИмяОбъектаМетаданных);
		НоваяСтрока_Информация.Данные = Ссылка;
		НоваяСтрока_Информация.Описание = ИмяФормыТестирования
			+ Символы.ПС + НСтр("ru = 'Открыть форму объекта по ссылке : '") + НавигационнаяСсылка
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		НоваяСтрока_Информация.Картинка = БиблиотекаКартинок.Справочник;	
	...
	// Окно не открылось
	// Возможно в обработчиках ПриСоздании или ПриОткрытии, флаг Отказ = Истина
	//Перенести в Аллюр со статусом Сломан
	Если Форма = Неопределено ИЛИ Форма.Окно = Неопределено Тогда
	...
	Исключение
		...
		НоваяСтрока_Информация = _Информация.Добавить();
		НоваяСтрока_Информация.ТипОшибки = "Справочники";
		НоваяСтрока_Информация.ПроверяемыйОбъект = Форма.ИмяФормы;
		НоваяСтрока_Информация.Данные = Ссылка;
...
Процедура Справочники_ПротестироватьЗаписьОбъектаНаСервере(СсылкаНаОбъект)
	...
	Исключение
		НоваяСтрока_Информация = _Информация.Добавить();
		НоваяСтрока_Информация.ТипОшибки = "Справочники";
		НоваяСтрока_Информация.ПроверяемыйОбъект = ОбъектПоСсылке.Метаданные().ПолноеИмя();
...
Функция СоздатьОписаниеШага(Знач РезультатШага)
	...
	ТипОшибки				= РезультатШага.ТипОшибки;
	Описание				= РезультатШага.Описание;
	КороткоеОписание		= Сред(Описание, 1, 150);
	Данные					= РезультатШага.Данные;
	ПроверяемыйОбъект		= РезультатШага.ПроверяемыйОбъект;
	
	ОписаниеШага 			= ОписаниеСценарияАллюр();
	
	ОписаниеШага.Вставить("name"				, КороткоеОписание);
	ОписаниеШага.Вставить("description"			, ТипОшибки);

	ВставитьОписаниеСтатусаАллюр(ОписаниеШага, Описание);

	ОписаниеШага.Вставить("status"				, СтатусАллюр());

	ОписаниеШага.historyId		= СтрШаблон("%1.%2.%3", ТипОшибки, ПроверяемыйОбъект, КороткоеОписание);
	ОписаниеШага.description	= СтрШаблон("%1.%2.%3", ТипОшибки, ПроверяемыйОбъект, Данные);

	СписокМеток = ОписаниеШага.labels;
	ДобавитьМеткуАллюр(СписокМеток, "host"		, ТипОшибки);
	ДобавитьМеткуАллюр(СписокМеток, "feature"	, ТипОшибки);
