...
Процедура ПриОткрытии(Отказ)
	...
	//++ РС 11.08.2021
	КоличествоДокументовПроведение		= 5;
	КоличествоДокументовПечатныеФормы	= 3;
	ЗакрыватьМодальныеОкна				= Истина;
	ТестированиеКомандногоИнтерфейса	= Ложь;
	ТестированиеПечатныхФорм			= Истина;
	ТестированиеПроведение				= Истина;
	ТестированиеМакетыСКД				= Истина;
	ТестированиеОткрытиеФорм			= Истина;
	ОткрываемФормыНаКлиентеТестирования	= Ложь;
	//-- РС
...
Процедура ЗаполнитьJSONНастроек(ЗаписьJSON)
	...
	//++ РС 11.08.2021
	Настройки.Вставить("Используется", ТестированиеОткрытиеФорм);
	Настройки.Вставить("ОткрываемФормыНаКлиентеТестирования", ОткрываемФормыНаКлиентеТестирования);

	СоответсвтиеФайла = Новый Соответствие;
	СоответсвтиеФайла.Вставить(КлючНастройки(), Настройки);
	
	ЗаписатьНастройкуПоКлючу(КлючНастройкиМодальныеОкна(), СоответсвтиеФайла);
	ЗаписатьНастройкуПоКлючу(КлючНастройкиПроведениеДокументов(), СоответсвтиеФайла);
	ЗаписатьНастройкуПоКлючу(КлючНастройкиФормированиеПечатныхФорм(), СоответсвтиеФайла);
	ЗаписатьНастройкуПоКлючу(КлючНастройкиМакетыСКД(), СоответсвтиеФайла);
	ЗаписатьНастройкуПоКлючу(КлючНастройкиКомандныйИнтерфейс(), СоответсвтиеФайла);
	ЗаписатьJSON(ЗаписьJSON, СоответсвтиеФайла)
	//-- РС
...
Процедура ЗагрузитьФайлНастроекНаСервере(Результат)
	...
	Исключение
		...
		СообщитьПользователю("Ошибка при загрузке настроек открытия форма из файла: " + ОписаниеОшибки());
		//-- РС
	КонецПопытки;
	...
	ТестированиеКомандногоИнтерфейса = ИспользуетсяНастройка(Результат, КлючНастройкиКомандныйИнтерфейс());
	ТестированиеМакетыСКД 			 = ИспользуетсяНастройка(Результат, КлючНастройкиМакетыСКД());
	ТестированиеПечатныхФорм		 = ИспользуетсяНастройка(Результат, КлючНастройкиФормированиеПечатныхФорм());
	ТестированиеПроведение			 = ИспользуетсяНастройка(Результат, КлючНастройкиПроведениеДокументов());
	ТестированиеОткрытиеФорм		 = ИспользуетсяНастройка(Результат, КлючНастройки());
	
	ОткрываемФормыНаКлиентеТестирования = ИспользуетсяНастройка(Результат, КлючНастройки(), "ОткрываемФормыНаКлиентеТестирования");
	//-- РС
...
Процедура ДобавитьМетаданныеВТаблицуМакетовСКД()
	...
	Для Каждого ОбщийМакет Из Метаданные.ОбщиеМакеты Цикл
		...
		СтрокаОбъекта.Объект			= ОбъектОбщиеМакеты();
	...
	Для Каждого ПроверяемыйОбъект Из ПроверяемыеОбъекты Цикл
		...
		Для Каждого ТекОбъект Из Метаданные[ПроверяемыйОбъект] Цикл
			...
			Для Каждого ТекДанныеМакета Из ТекОбъект.Макеты Цикл
				...
				СтрокаОбъекта.ИмяМетаданного	= ИмяОтчетыОбъекта();
...
Процедура ОбойтиУровеньДерева(Строки)
	...
	Для Каждого Строка из Строки Цикл
		...
		Если НЕ Строка.ИмяМетаданного = "" Тогда
			Строка.Исключено = НЕ Строка.ЕстьИзмененияРасширениями;
		КонецЕсли;
...
#Область дфЗаписьНастроекПоКлючам

&НаСервере
Функция ЗаписатьНастройкуПоКлючу(Ключ, СоответствиеНастроек)
...
Процедура ЗаписатьНастройкуПроведениеДокументов(СоответствиеНастроек)
	...
	НастройкаПроведениеДокументов.Вставить("Используется", 			ТестированиеПроведение);
...
Процедура ЗаписатьНастройкуФормированиеПечатныхФорм(СоответствиеНастроек)
	...
	НастройкаФормированиеПечатныхФорм.Вставить("Используется", 					ТестированиеПечатныхФорм);
	НастройкаФормированиеПечатныхФорм.Вставить("КоличествоДокументов",			КоличествоДокументовПечатныеФормы);
	НастройкаФормированиеПечатныхФорм.Вставить("ИсключенияПоОбъекту", 			МассивИсключений);
...
Процедура ЗаписатьНастройкуМакетыСКД(СоответствиеНастроек)
	...
	Для Каждого СтрокаИсключение Из СтрокиИсключения Цикл
		Если СтрокаИсключение.Объект = ОбъектОбщиеМакеты() Тогда
	...
	НастройкаМакетыСКД.Вставить("Используется", 				ТестированиеМакетыСКД);
...
Процедура ЗаписатьНастройкуКомандныйИнтерфейс(СоответствиеНастроек)
	...
	НастройкиФорм	= СоответствиеНастроек.Получить(КлючНастройки());
	
	НастройкаКомандныйИнтерфейс = Новый Соответствие;
	НастройкаКомандныйИнтерфейс.Вставить("Используется", ТестированиеКомандногоИнтерфейса);
	НастройкаКомандныйИнтерфейс.Вставить("СтрогийПорядокВыполнения", Истина);

	Для Каждого ЭлементНастройки Из НастройкиФорм Цикл
	...
	ИсключенияРазделы.Добавить("ФормированиеПечатныхФорм");
...
#Область дфЗагрузкаНастроекПоКлючам
...
Процедура ЗагрузитьНастройкиВТаблицу(Результат, Ключ);
	...
	Попытка
		...
		Для каждого Элемент из Настройки Цикл
			Если Ключ = КлючНастройкиПроведениеДокументов() Тогда
			 ...
			 ИначеЕсли Ключ = КлючНастройкиФормированиеПечатныхФорм() Тогда
				...
				Исключения = Новый Массив; 
				Настройки.Свойство("ИсключенияПоОбъекту",Исключения); 
				УстановитьПризнакИсключенияДокумента(Исключения, "ИсключеноПечатныеФормы");
...
Процедура УстановитьПризнакИсключенияМакетаСКД(ИсключенияОбщихМакетов, ИсключенияПоИмениМетаданных)
	...
	Для Каждого СтрокаИсключение Из ИсключенияОбщихМакетов Цикл
		СтрокиДокумента = ТаблицаМакетыСКД.НайтиСтроки(Новый Структура("Объект, ИмяМетаданного", 
			ОбъектОбщиеМакеты(), СтрокаИсключение));
		Если СтрокиДокумента.Количество() > 0 Тогда
			СтрокиДокумента[0].Исключено = Истина;
		Иначе
			НоваяСтрока 				= ТаблицаМакетыСКД.Добавить();
			НоваяСтрока.ИмяМетаданного	= СтрокаИсключение;
			НоваяСтрока.Объект			= ОбъектОбщиеМакеты();
	...
	Для Каждого СтрокаИсключение Из ИсключенияПоИмениМетаданных Цикл
		СтрокиДокумента = ТаблицаМакетыСКД.НайтиСтроки(Новый Структура("Объект, ИмяМетаданного", 
			СтрокаИсключение, ИмяОтчетыОбъекта()));
		Если СтрокиДокумента.Количество() > 0 Тогда
			СтрокиДокумента[0].Исключено = Истина;
		Иначе
			НоваяСтрока 				= ТаблицаМакетыСКД.Добавить();
			НоваяСтрока.ИмяМетаданного	= ИмяОтчетыОбъекта();
...
Функция ОбъектОбщиеМакеты()
	Возврат "ОбщиеМакеты";
КонецФункции

Функция ИмяОтчетыОбъекта()
	Возврат "ОтчетыОбъекта";
КонецФункции

Функция ИспользуетсяНастройка(Настройки, КлючТеста, КлючНастройки = "Используется")
	
	Возврат Настройки.Свойство(КлючТеста) 
		И Настройки[КлючТеста].Свойство(КлючНастройки)
		И Настройки[КлючТеста][КлючНастройки];
